...
function getLanInterfaces() {
  ...
  for (const iface of Object.values(ifaces)) {
    for (const addr of iface) {
      ...
      if (LOCAL_IP && !sameSubnet(addr.address, LOCAL_IP, addr.netmask))
        continue;
...
socket.on("message", (msg, rinfo) => {
  ...
  // Single cache-flush packet containing one Answer RR per IP
  // (covers WiFi + Ethernet simultaneously without cache overwrite)
  const response = buildResponse(DOMAIN, ips, q.id);
  socket.send(response, 0, response.length, MDNS_PORT, MDNS_ADDR, (err) => {
    if (err) console.error(`❌ Send error (${ips.join(", ")}):`, err.message);
  });
});
